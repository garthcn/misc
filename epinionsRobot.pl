...
sub fetch_user {
    ...
    while ($continue_fetching_reviews_pages == 1) {
        ...
        my $user_reviews_page_url = 'http://www.epinions.com/user-'.$user_name_string.'/sec_~public_profile_opinion_list/show_~content/pp_~'.$reviews_page_index++.'/pa_~1#list';
        my $req = HTTP::Request->new(POST => $user_reviews_page_url);
        # print "\n";
        # print $user_reviews_page_url;
        # print "\n"; 
        ...
        if (index($cont,'Date&nbsp;Written') < 0) {
            ...
            print "Fetching page ".$reviews_page_index."\n";
